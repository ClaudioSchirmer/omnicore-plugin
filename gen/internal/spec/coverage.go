	...
	for i, r := range rs.List {
		...
		if r.SkipWhen != "" {
			ps.BlockerFix(w+".skipWhen",
				"skipWhen is not generated by this build",
				"drop it, or move the rule to rules.manual")
		}
		if len(r.Transitions) > 0 {
			ps.BlockerFix(w+".transitions",
				"state transitions are not generated by this build",
				"move the rule to rules.manual, where it becomes a named item in the "+
					"report and a stub in the hook file")
		}
		if len(r.GroupBy) > 0 || r.Cap > 0 {
			ps.BlockerFix(w+".groupBy",
				"per-group caps are not generated by this build",
				"move the rule to rules.manual — counting rows the aggregate cannot "+
					"see needs a service fact, and the rule that reads it is yours to write")
		}
		if r.AdminField != "" {
			ps.BlockerFix(w+".adminField",
				"an administrator bypass is not generated by this build",
				"drop it, or move the rule to rules.manual")
		}
	}
