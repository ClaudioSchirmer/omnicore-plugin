...
source a generator can have: today that is the framework's **v0.55.0**, paired with plugin
**0.25.0**. Publish the two in sync. An older pin is refused by default with the fix named;
...
specs, reports and lock (`specs/omnicore-gen/`), the contract suite (`specs/qa/`), the
...
keeps what was decided about it.
...
(`plugins/omnicore/shared/generated-documents.md` is the full rule).
