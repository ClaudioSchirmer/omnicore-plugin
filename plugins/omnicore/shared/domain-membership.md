...
| a persisted resource whose rules are not the aggregate's | still a domain struct, with an **empty `BuildRules`** — the write-backed schema is type-anchored (`notification-bases.md`, "the adjacent trap"). Placement out is never the conclusion for a persisted type |
