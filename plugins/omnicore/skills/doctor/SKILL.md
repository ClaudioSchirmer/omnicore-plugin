...
| an unarchive that restored NO child, or revived one removed long before the root's archive — the restore is scoped by the archive STAMP, so the suspects are an `archived_at` coarser on the child than on the root (it truncates the stamp and matches nothing) and a second-resolution column (a bare MySQL `DATETIME`, which folds two operations of the same second into one). Neither raises an error | aggregate-persistence · table-schema |
