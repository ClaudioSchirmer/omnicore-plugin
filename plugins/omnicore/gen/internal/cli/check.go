...
// neighboursOf hands the validator what the project's other specs already
// claim, so a name collision is refused here rather than at boot.
func neighboursOf(p *discover.Project) []spec.Neighbour {
	...
	for _, c := range p.SiblingSpecs {
		n := spec.Neighbour{
			...
			Table: c.Table, Revision: c.Revision,
		}
