...
type File struct {
	...
	Consequence string
	// Shared marks a file that belongs to the PROJECT rather than to the entity
	// that happened to generate it: every spec producing it produces the same
	// bytes. It changes only the header — a shared file names no entity and no
	// spec — but that is the whole point. Attributed to whichever spec ran last,
	// such a file is rewritten by every generation of every OTHER entity, so a
	// project with two entities can never reach a state where regenerating
	// changes nothing.
	//
	// It is not a fourth Class: the file is still Owned, still hashed, still
	// refused when edited by hand. Only the attribution is different.
	Shared bool
}
...
// claimedByOther names another entity of this project that also records the
// path, or "" when this entity is the only claimant.
//
// Two situations reach it, and neither is this entity's to clean up. A SHARED
// file — the vos package comment — is produced identically by every spec that
// has a value object; and any file two specs happen to produce is on disk once.
// Dropping the last field that made THIS spec produce it does not make the file
// a leftover: another spec still generates it, and deleting it is a broken
// build in a package the author was not touching.
//
// The names are sorted so the reason a reader is given does not depend on map
// iteration order.
func (l *Lock) claimedByOther(entity, path string) string {
	if l == nil {
		return ""
	}
	var names []string
...
func Orphans(entity string, files []File, lock *Lock) []string {
	...
	for path := range prev.Files {
		// Still generated by a NEIGHBOUR is not orphaned. Reporting it would
		// invite a prune that takes a file another spec still writes.
		if !now[path] && lock.claimedByOther(entity, path) == "" {
			out = append(out, path)
		}
	}
	sort.Strings(out)
...
func PlanPrune(root, entity string, current []File, lock *Lock) []PruneFile {
	...
	for path, rec := range prev.Files {
		...
		content, err := os.ReadFile(filepath.Join(root, path))
		switch {
		case err == nil && lock.claimedByOther(entity, path) != "":
			// The same rule the registration prune already applies one
			// declaration at a time: a thing another entity of this project also
			// produces is not this run's to remove. Reported rather than
			// silently skipped, because "why is that file still here?" is a
			// question the author will otherwise ask the compiler.
			out = append(out, PruneFile{path, PruneKeep,
				"another entity of this project generates it too (" +
					lock.claimedByOther(entity, path) + ")"})
		case err != nil:
