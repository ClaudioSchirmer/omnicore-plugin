...
// UpdatedAt and ArchivedAt unexported, so FieldByName finds nothing and the check
...
// archived_at IS NULL being the normal state of a row, not the exception.
//
...
func TestAJoinedArchiveColumnLandsInAPointer(t *testing.T) {
	target := &discover.SpecClaim{
		...
		Fields: []discover.FieldClaim{
			...
			{Name: "ArchivedAt", Column: "archived_at", Type: "time", LivesOn: "root", Nullable: true},
		},
	}
