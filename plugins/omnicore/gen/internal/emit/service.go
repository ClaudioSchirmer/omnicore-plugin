...
//
// INSIDE the port's file, above the interface that names them. They are not
// domain types in their own right — nothing models them, nothing else may pass
// them — they are the parameters and returns of these methods, and a reader
// meets them one line before the signature that reads them. A file each spelled
// their names in full (`<entity><fact>entry.go`), which put three lines of Go
// under twenty-six of header and left the domain package quoting fact names in
